...
7. `38 North`, `CSIS Korea Chair`처럼 소스 자체가 한반도/북한
   ...
   관련도를 부여합니다.
8. 카카오/코코아 농작물 문맥처럼 알려진 오탐 패턴을 제외합니다.
9. `high` confidence 신규/업데이트 항목만 텔레그램 채널로 보냅니다.
10. `data/state.json`과 `data/history/*.ndjson`를 갱신하고 커밋합니다.
11. 각 모니터 실행 결과를 `data/run_logs/*.ndjson`에 저장해 주간
    건강점검에 활용합니다.
