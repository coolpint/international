...
- `certifi==2026.7.22` (외부 피드 HTTPS 인증서 검증용 CA 번들)
...
- 새 소스는 해외 정부·국제기구·공공기관·원발표 기관의 공식 원문을
  우선합니다.
- 한국 언론사와 한국 언론의 해외판은 수집하지 않습니다.
- 한국 언론사명을 인용 근거로 명시한 항목은 필터에서 제외합니다.
- 뉴욕타임스와 월스트리트저널은 사용자 지정 고우선 예외입니다. 두
  Google News 검색 피드는 `<source>` 이름과 도메인을 모두 검증하며,
  검증된 항목이 0건이면 소스 오류로 기록합니다.
- 이 두 매체의 한국 관련 결과는 `high`로 강제하지만, 한국 언론
  재인용이면 다른 소스와 동일하게 제외합니다. 이 제외 원칙이 미디어
  우선순위보다 상위입니다.
- NYT·WSJ 유료 본문은 GitHub Actions의 비로그인 요청에서 각각
  403·401을 반환하므로, 한국 언론사명과 인용 표현을 함께 검색하는
  언론사별 짧은 Google News RSS의 동일 GUID를 합쳐 제외합니다. 제외
  RSS가 실패하거나 항목을 반환하면서 대상 발행처 검증 결과가 0건이면
  검색 의미가 흐트러진 것으로 보고 해당 매체 수집도 fail-closed로
  실패합니다. 제외 검색어는 `keywords.json`의 한국 언론 출처명
  목록에 포함되는지를 회귀 테스트로 고정합니다.
- `high` 신규·업데이트 알림은 해당 내용의 전송 성공 hash가 기록될
  때까지 다음 스케줄에서 재시도합니다. 첫 bootstrap 기준선은 재시도
  대상이 아닙니다.
- 개인 Chrome 로그인 세션은 GitHub Actions 런타임과 분리돼 있으므로
  수집기에 쿠키나 세션을 복사하지 않습니다. 사용자가 알림 링크를
  Chrome에서 열 때는 기존 NYT·WSJ 유료 로그인을 그대로 사용할 수
  있습니다.
- 다국어 소스를 추가할 때 해당 언어의 한국·북한·한반도 표현도 함께
  추가해야 합니다.
- 기업 키워드는 정식 법인명과 충분히 고유한 현지어명을 사용하며
  `SK`, `LG`, `KT`, `Kia` 같은 모호한 단독 약어는 사용하지 않습니다.
- 세부 기준은 [`constitution.md`](../constitution.md), 진행 및 검증
  기록은 [`plan.md`](../plan.md)를 확인합니다.
